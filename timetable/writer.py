"""Write the styled workbook: 'Class Time table' + 'Teacher Time Table'.

Format mirrors NRHS_Sample_Timetable.xlsx (navy headers, blue day column,
"Teacher\\n(SUBJ)" cells, study-hour row, per-teacher blocks).
"""
...
from .model import (Model, DAYS, N_DAYS, STUDY_PERIOD, SUBJ_ABBR, CLASS_DISPLAY,
                    PARALLEL_SUBJECTS, GENERIC_TEACHER)
...
ROW_LABELS = [1, 2, 3, 4, 5, 6, 7, "Study Hour"]     # 8 rows per day


def _abbr(subj):
    return SUBJ_ABBR.get(subj, subj[:4].upper())
...
def write_class_sheet(wb, m: Model, solution: dict):
    ws = wb.create_sheet("Class Time table")
    # header
    _cell(ws, 1, 1, "Day", NAVY, WHITE_BOLD)
    _cell(ws, 1, 2, "Period", NAVY, WHITE_BOLD)
    for j, c in enumerate(m.classes, start=3):
        _cell(ws, 1, j, CLASS_DISPLAY.get(c, c), NAVY, WHITE_BOLD)
    ...
    for d in range(N_DAYS):
        ...
        for label in ROW_LABELS:
            ...
            row += 1
        # merge day column across the 8 rows, rotated
...
def write_teacher_sheet(wb, m: Model, solution: dict):
    ws = wb.create_sheet("Teacher Time Table")
    _cell(ws, 1, 1, "SCHOOL TEACHER TIMETABLE", NAVY, Font(bold=True, color="FFFFFF", size=13))
    ws.merge_cells("A1:I1")

    # build per-teacher grid: tgrid[t][d][p] -> text  (p 1..8, 8 = study hour col)
    ...
    real = [t for t in m.teachers]
    generic = [g for g in GENERIC_TEACHER.values() if any(tt == g for _, tt in solution.values())]
    order = real + generic
    ...
    for t in order:
        ...
        row += 1
        heads = ["DAY", "PERIOD 1", "PERIOD 2", "PERIOD 3", "PERIOD 4",
                 "PERIOD 5", "PERIOD 6", "PERIOD 7", "STUDY HOUR"]
...
def write_workbook(path: str, m: Model, solution: dict):
    wb = openpyxl.Workbook()
    wb.remove(wb.active)
    write_teacher_sheet(wb, m, solution)      # sample orders Teacher sheet first
